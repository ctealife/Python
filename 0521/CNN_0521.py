...
BASE_DIR = r"C:\鈞論文\paper_2026\CNN\training_result\0521"

TOPOLOGY_CONFIG = {
    "2T2R": {
        "active_tx": [1, 2],
        "active_rx": [1, 2],
        "data_dir": os.path.join(BASE_DIR, "2T2R"),
    },
    "2T1R": {
        "active_tx": [1, 2],
        "active_rx": [1],
        "data_dir": os.path.join(BASE_DIR, "2T1R"),
    },
    "1T2R": {
        "active_tx": [1],
        "active_rx": [1, 2],
        "data_dir": os.path.join(BASE_DIR, "1T2R"),
    },
    "1T1R": {
        "active_tx": [1],
        "active_rx": [1],
        "data_dir": os.path.join(BASE_DIR, "1T1R"),
    },
}

if TOPOLOGY not in TOPOLOGY_CONFIG:
    raise ValueError(f"未知 TOPOLOGY: {TOPOLOGY}")

cfg = TOPOLOGY_CONFIG[TOPOLOGY]

active_tx = cfg["active_tx"]
active_rx = cfg["active_rx"]

active_tx_idx = [t - 1 for t in active_tx]   # Python zero-based
active_rx_idx = [r - 1 for r in active_rx]

num_tx = len(active_tx)
...
mat_file_paths = {
    rx_id: os.path.join(dir_path, f"training_data_rx{rx_id}.mat")
    for rx_id in active_rx
}

model_dir = os.path.join(dir_path, f"Training_Result_{TOPOLOGY}")
os.makedirs(model_dir, exist_ok=True)

print(f"[Info] TOPOLOGY = {TOPOLOGY}")
print(f"[Info] active_tx = {active_tx}")
print(f"[Info] active_rx = {active_rx}")
print(f"[Info] num_paths = {num_paths}")
print(f"[Info] data_dir = {dir_path}")
...
def read_mat73_dataset(h5file, key, dtype=None, squeeze=True):
    """
    讀取 MATLAB v7.3 MAT 檔中的單一 numeric dataset。

    注意：
    feature_4d 必須保留 singleton pulse dimension（單一 pulse 維度），
    因此讀取 feature_4d 時要設定 squeeze=False。
    """
...
def load_mat73_selected(mat_path, required_keys, optional_keys=None, verbose=False):
    ...
    with h5py.File(mat_path, "r") as f:
        for key in required_keys:
            t0 = time.time()
            data[key] = read_mat73_dataset(
                f,
                key,
                squeeze=(key not in ["feature_4d", "S_clean_4d"])
            )
...
rx1_required_keys = base_required_keys + [
    "target_x_vec",
    "target_y_vec",
    "SCNR_dB_vec",
    "snr_vec",
...
# PN 只讀取 active Tx 需要的 PN
if 1 in active_tx:
    rx1_required_keys.append("pnSeq1")
if 2 in active_tx:
    rx1_required_keys.append("pnSeq2")
...
for rx_id in active_rx:
    mat_path = mat_file_paths[rx_id]

    if not os.path.exists(mat_path):
        raise FileNotFoundError(
            f"TOPOLOGY={TOPOLOGY} 需要 Rx{rx_id} MAT 檔案，但找不到: {mat_path}"
        )

    if rx_id == active_rx[0]:
        required_keys = rx1_required_keys
    else:
        required_keys = base_required_keys + same_metadata_keys

    print(f"[Info] 載入 Rx{rx_id} MAT 檔案: {mat_path}", flush=True)
    t0 = time.time()

    data_by_rx[rx_id] = load_mat73_selected(
        mat_path,
        required_keys=required_keys,
        optional_keys=rx_optional_keys,
        verbose=False
    )

    print(f"[Info] Rx{rx_id} 載入完成，耗時 {time.time() - t0:.2f} 秒", flush=True)

# 以第一個 active Rx 作為 metadata reference
ref_rx = active_rx[0]
data_ref = data_by_rx[ref_rx]

# ---------------- 組合 MIMO input: (N, Rx, P, L, I/Q) ----------------

X_list = []

for rx_id in active_rx:
    X_rx = data_by_rx[rx_id].pop("feature_4d").astype("float32", copy=False)

    if X_rx.ndim != 4:
        raise ValueError(f"Rx{rx_id} feature_4d 應為 4D (N,P,L,2)，但得到 {X_rx.shape}")

    X_list.append(X_rx)

# 檢查各 Rx feature shape 是否一致
for k in range(1, len(X_list)):
    if X_list[k].shape != X_list[0].shape:
        raise ValueError(
            f"不同 Rx 的 feature_4d shape 不一致: "
            f"{X_list[0].shape} vs {X_list[k].shape}"
        )

X = np.stack(X_list, axis=1).astype("float32", copy=False)
# shape: (N, num_rx, P, L, 2)

del X_list
gc.collect()


# ---------------- label / sample_id / metadata consistency check ----------------

label_ref = np.array(data_ref["label_bin"]).squeeze().astype("float32")
sample_id_ref = np.array(data_ref["sample_id"]).squeeze().astype("int32")

for rx_id in active_rx:
    data_now = data_by_rx[rx_id]

    label_now = np.array(data_now["label_bin"]).squeeze().astype("float32")
    sample_id_now = np.array(data_now["sample_id"]).squeeze().astype("int32")

    if not np.array_equal(label_ref, label_now):
        raise ValueError(f"Rx{ref_rx} 與 Rx{rx_id} 的 label_bin 不一致。")

    if not np.array_equal(sample_id_ref, sample_id_now):
        raise ValueError(f"Rx{ref_rx} 與 Rx{rx_id} 的 sample_id 不一致。")

    if rx_id != ref_rx:
        for key in same_metadata_keys:
            if key not in data_ref or key not in data_now:
                raise KeyError(f"Rx{ref_rx} 或 Rx{rx_id} MAT 檔中找不到 {key}。")

            v_ref = np.array(data_ref[key]).squeeze()
            v_now = np.array(data_now[key]).squeeze()

            if not np.allclose(v_ref, v_now, equal_nan=True):
                raise ValueError(
                    f"Rx{ref_rx} 與 Rx{rx_id} 的 {key} 不一致，代表樣本 metadata 錯位。"
                )

y_det = label_ref.astype("float32")
...
if "SCNR_dB_vec" not in data_ref:
    raise KeyError("MAT 檔中找不到 SCNR_dB_vec。請確認 MATLAB 端已儲存 global clutter-based SCNR。")

scnr_global_vec = np.array(data_ref["SCNR_dB_vec"]).squeeze().astype("float32")

# Nominal SCNR metadata：MATLAB 端指定的 SCNR 排程值，例如 -80:10:10
if "SCNR_nominal_dB_vec" in data_ref:
    scnr_nominal_vec = np.array(data_ref["SCNR_nominal_dB_vec"]).squeeze().astype("float32")
else:
    scnr_nominal_vec = np.full_like(scnr_global_vec, np.nan, dtype="float32")

# SNR metadata
if "snr_vec" not in data_ref:
    raise KeyError("MAT 檔中找不到 snr_vec。")

snr_vec = np.array(data_ref["snr_vec"]).squeeze().astype("float32")

# RCS metadata
if "rcs_vec" not in data_ref:
    raise KeyError("MAT 檔中找不到 rcs_vec。")

rcs_vec = np.array(data_ref["rcs_vec"]).squeeze().astype("float32")

# no-target noise RCS metadata
if "noise_rcs_vec" in data_ref:
    noise_rcs_vec = np.array(data_ref["noise_rcs_vec"]).squeeze().astype("float32")
else:
    noise_rcs_vec = np.full_like(rcs_vec, np.nan, dtype="float32")

# range class metadata
if "label_range_cls" in data_ref:
    range_cls = np.array(data_ref["label_range_cls"]).squeeze().astype("int32")
else:
...
# active topology 對應的 Tx-Rx bistatic path length
# flatten order: for tx in active_tx, for rx in active_rx
# 2T2R: Tx1Rx1, Tx1Rx2, Tx2Rx1, Tx2Rx2
# 2T1R: Tx1Rx1, Tx2Rx1
# 1T2R: Tx1Rx1, Tx1Rx2
# 1T1R: Tx1Rx1
rbi_raw = np.stack([
...
if rbi_raw.shape[1] != num_paths:
    raise ValueError(
        f"rbi_raw path 數量錯誤，預期 {num_paths}，得到 {rbi_raw.shape[1]}"
    )

# radar geometry
tx_pos_all_full = np.array(data_ref["tx_pos_all"]).astype("float32")  # (2, 2)
rx_pos_all_full = np.array(data_ref["rx_pos_all"]).astype("float32")  # (2, 2)

tx_pos_active = tx_pos_all_full[active_tx_idx, :]
rx_pos_active = rx_pos_all_full[active_rx_idx, :]

# PN codes：依 active_tx 決定使用 PN1 / PN2
pnSeq_by_tx = {}

if 1 in active_tx:
    pnSeq_by_tx[1] = np.array(data_ref["pnSeq1"]).squeeze().astype("float32")

if 2 in active_tx:
    pnSeq_by_tx[2] = np.array(data_ref["pnSeq2"]).squeeze().astype("float32")

pnSeq_active = [pnSeq_by_tx[tx_id] for tx_id in active_tx]
if len(pnSeq_active) != num_tx:
    raise ValueError(
        f"PN 數量與 num_tx 不一致，len(pnSeq_active)={len(pnSeq_active)}, num_tx={num_tx}"
    )
...
for tx_id, pn in zip(active_tx, pnSeq_active):
    print(f"len(pnSeq{tx_id}) = {len(pn)}")

if len(set(pn_lengths)) != 1:
    raise ValueError(f"active PN 長度不一致: {pn_lengths}")
...
if num_rx != num_rx_expected:
    raise ValueError(
        f"num_rx 與 TOPOLOGY 不一致，預期 {num_rx_expected}，但得到 {num_rx}"
    )

if num_features != 2:
    raise ValueError(f"num_features 應為 2 (I/Q)，但得到 {num_features}")

print(f"[Info] X topology input shape: {X.shape}")  # (N, Rx, P, L, 2)
print(f"[Info] active_tx={active_tx}, active_rx={active_rx}, num_paths={num_paths}")
...
class OneLineBatchProgress(tf.keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        # 只保留訓練進度條，不顯示 train loss / train metric
        step = batch + 1
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        val_rbi_mae = logs.get("val_rbi_output_mae", np.nan)
...
# 2T2R 時 rbi_min/rbi_max shape = (4,)
# 2T1R 或 1T2R 時 shape = (2,)
# 1T1R 時 shape = (1,)
rbi_min = np.nanmin(
    rbi_raw[idx_train][train_target_mask_for_norm],
    axis=0
).astype("float32")
...
# 每一條 Tx-Rx path 都有自己的正規化範圍，不再把四條路徑混在同一個 min/max 中
...
if np.sum(val_target_mask_for_check) > 0:
    ...
    val_rbi_norm_check = (
        (rbi_raw[idx_val][val_target_mask_for_check] - rbi_min) /
        (rbi_max - rbi_min + 1e-8)
    )

    print("[Check] val normalized RBI min:", np.nanmin(val_rbi_norm_check, axis=0))
    print("[Check] val normalized RBI max:", np.nanmax(val_rbi_norm_check, axis=0))

    if (np.nanmin(val_rbi_norm_check) < -0.05) or (np.nanmax(val_rbi_norm_check) > 1.05):
        print("[Warning] validation RBI label 明顯超出 training normalization range。")
...
print(f"[Info] rbi_min per-path = {rbi_min}")
print(f"[Info] rbi_max per-path = {rbi_max}")
print(f"[Info] y_pos shape: {y_pos.shape}")
print(f"[Info] y_geo shape: {y_geo.shape}")

# ---------------- Model ----------------
def build_mimo_model(
    num_tx, num_rx, num_pulses, sequence_length, num_features,
    pnSeq_active, pos_min, pos_max,
    tx_pos_active, rx_pos_active, rbi_min, rbi_max,
    cfar_win_len, cfar_guard_len, topology_name
):
    ...
    if len(pn_len_set) != 1:
        raise ValueError("active PN 長度不一致。")

    kernel_size = len(pn_mf_list[0])

    # 每個 Tx 對應 I/Q 兩個 filter，因此 filters = 2 * num_tx
    init_weight = np.zeros((kernel_size, 2, 2 * num_tx), dtype=np.float32)

    for k, pn_mf in enumerate(pn_mf_list):
        init_weight[:, 0, 2 * k]     = pn_mf  # I
        init_weight[:, 1, 2 * k + 1] = pn_mf  # Q
    ...
    # independent bistatic range output:
    # this is learned directly from CNN features, not computed from pos_output
    rbi_output = Dense(num_paths, activation="sigmoid", name="rbi_output")(x)
    ...
    tx_tf = tf.constant(tx_pos_active, dtype=tf.float32)  # (num_tx, 2)
    rx_tf = tf.constant(rx_pos_active, dtype=tf.float32)  # (num_rx, 2)

    rbi_min_tf = tf.constant(rbi_min.reshape(1, num_paths), dtype=tf.float32)
    rbi_max_tf = tf.constant(rbi_max.reshape(1, num_paths), dtype=tf.float32)
    ...
    def geo_from_pos(pos_norm):
        ...
        # flatten order 與 rbi_raw 一致：
        # for tx in active_tx, for rx in active_rx
    ...
    model = Model(
        ...
        name=f"mimo_{topology_name}_det_pos_rbi_geo"
    )
...
# normalized bistatic range -> physical bistatic range
rbi_val_pred_norm = np.clip(rbi_val_pred_norm, 0.0, 1.0)
rbi_val_pred = rbi_val_pred_norm * (rbi_max - rbi_min) + rbi_min
...
def scnr_percentile_localization_report(
    scnr_global_db,
    y_true,
    y_pred,
    pos_pred,
    ...
    """
    依照 target samples 的 Global SCNR 由低到高排序，
    輸出 0~100% 整體與四個 SCNR 百分位區段的定位誤差。

    統計對象：
        只使用 y_true == 1 且 SCNR 為 finite 的 validation target samples。

    輸出：
        X MAE
        Y MAE
        Position Euclidean MAE
        每條 bistatic path 的 RBI MAE
        RBI Mean MAE
    """
    ...
    print(f"\n========== {tag} SCNR-percentile Localization Metrics ==========")

    if len(valid_idx) == 0:
        print(f"[{tag}] 沒有 finite Global SCNR 的 target samples，無法計算 SCNR-percentile localization metrics。")
        return

    # 依 Global SCNR 由低到高排序
    valid_idx = valid_idx[np.argsort(scnr_global_db[valid_idx])]

    def _report_one_group(group_name, idx_group):
        if len(idx_group) == 0:
            print(f"\n[{group_name}] count = 0，略過。")
            return

        scnr_now = scnr_global_db[idx_group]

        # ---------- Detection metrics ----------
        y_true_group = y_true[idx_group]
        ...
        target_idx_group = idx_group[y_true[idx_group] == 1]

        if len(target_idx_group) > 0:
            pos_err = pos_pred[target_idx_group] - pos_true[target_idx_group]
            rbi_err = rbi_pred[target_idx_group] - rbi_true[target_idx_group]

            x_mae = np.mean(np.abs(pos_err[:, 0]))
            y_mae = np.mean(np.abs(pos_err[:, 1]))
            pos_euclidean_mae = np.mean(np.linalg.norm(pos_err, axis=1))

            rbi_mae_path = np.mean(np.abs(rbi_err), axis=0)
            rbi_mae_all = np.mean(np.abs(rbi_err))
        else:
            x_mae = np.nan
            y_mae = np.nan
            pos_euclidean_mae = np.nan
            rbi_mae_path = np.full(len(selected_rbi_keys), np.nan)
            rbi_mae_all = np.nan
        ...
        print(f"\n[{group_name}]")
        print(f"count              : {len(idx_group)}")
        print(f"target count       : {np.sum(y_true_group == 1)}")
        print(f"no-target count    : {np.sum(y_true_group == 0)}")
        print(f"SCNR min / median / max : "
            f"{np.nanmin(scnr_now):.2f} / {np.nanmedian(scnr_now):.2f} / {np.nanmax(scnr_now):.2f} dB")

        print(f"[{tag}]   precision: {precision:.4f}, recall/Pd: {recall:.4f}, F1: {f1:.4f}")
        print(cm)

        print(f"X MAE              : {x_mae:.4f} m")
        print(f"Y MAE              : {y_mae:.4f} m")
        print(f"Position Euclidean MAE : {pos_euclidean_mae:.4f} m")

        for i, key in enumerate(selected_rbi_keys):
            print(f"{key} MAE          : {rbi_mae_path[i]:.4f} m")

        print(f"RBI Mean MAE       : {rbi_mae_all:.4f} m")

    # 原本 0~100% 結果保留，這裡額外再輸出一份同格式總表，方便與四分位比較。
    _report_one_group("SCNR 0~100%", valid_idx)

    # 四等分：SCNR 最低 25%、25~50%、50~75%、最高 25%
    quartile_groups = np.array_split(valid_idx, 4)
    quartile_names = [
        "SCNR 0~25%",
        "SCNR 25~50%",
        "SCNR 50~75%",
    ...
    for name, idx_group in zip(quartile_names, quartile_groups):
        _report_one_group(name, idx_group)
...
val_rbi_err = rbi_val_pred[val_target_mask] - rbi_raw_val[val_target_mask]
...
val_rbi_mae_path = np.mean(np.abs(val_rbi_err), axis=0)
val_rbi_rmse_path = np.sqrt(np.mean(val_rbi_err ** 2, axis=0))
val_rbi_mae_all = np.mean(np.abs(val_rbi_err))
val_rbi_rmse_all = np.sqrt(np.mean(val_rbi_err ** 2))
...
print("\n========== Bistatic Range Regression Metrics ==========")
for i, key in enumerate(selected_rbi_keys):
    print(
        f"[Val]   {key} MAE: {val_rbi_mae_path[i]:.4f} m, "
        f"RMSE: {val_rbi_rmse_path[i]:.4f} m"
    )

print(f"[Val]   RBI Mean MAE: {val_rbi_mae_all:.4f} m")
print(f"[Val]   RBI Mean RMSE: {val_rbi_rmse_all:.4f} m")

scnr_percentile_localization_report(
    scnr_global_db=scnr_global_val,
    y_true=det_val.squeeze(),
    y_pred=det_val_pred,
    pos_pred=pos_val_pred,
    pos_true=pos_raw_val,
    rbi_pred=rbi_val_pred,
    rbi_true=rbi_raw_val,
    selected_rbi_keys=selected_rbi_keys,
    tag="Val"
)
...
plt.figure(figsize=(6, 4))
plt.plot(history.history["rbi_output_mae"], label="train_rbi_mae")
plt.plot(history.history["val_rbi_output_mae"], label="val_rbi_mae")
plt.xlabel("Epoch")
plt.ylabel("Normalized Bistatic Range MAE")
plt.legend()
plt.title("Bistatic Range Regression MAE")
plt.savefig(os.path.join(model_dir, "rbi_mae.png"))
plt.close()
...
weights_path = os.path.join(model_dir, f"{TOPOLOGY}_det_pos_rbi_geo_weights.h5")
norm_path = os.path.join(model_dir, f"{TOPOLOGY}_norm_params.pkl")
...
with open(norm_path, "wb") as f:
    pickle.dump({
        ...
        "rbi_min": rbi_min,
        "rbi_max": rbi_max,
        "tx_pos_all_full": tx_pos_all_full,
        "rx_pos_all_full": rx_pos_all_full,
        "tx_pos_active": tx_pos_active,
        "rx_pos_active": rx_pos_active,
        "selected_rbi_keys": selected_rbi_keys,
    }, f)
